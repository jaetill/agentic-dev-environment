...
> **Label-write trust boundary (accepted risk):** `source:sentry`, `source:cloudwatch`, and `severity:critical` share the same label-write trust surface — any repo collaborator with Triage-role access or higher can apply any of these labels to an arbitrary open issue and trigger autonomous implementation without the `ready-for-implementer` gate.
