...
- `origin:sentry` (Sentry-originated production bug — Sentry's GitHub integration auto-applies this label when its alert rules create issues; these always get implementer attention per ADR-0016; production errors that fired in real users' sessions are pre-validated work)
- `origin:cloudwatch` (CloudWatch-originated alert — per [ADR-0023](../../../docs/adr/0023-origin-based-autonomy-boundary.md), machine-detected breakage from CloudWatch alarms gets the same autonomous pickup as Sentry)
- `severity:critical` (critical-severity finding from any reviewer agent)

> **Label-write trust boundary (accepted risk):** `origin:sentry`, `origin:cloudwatch`, and `severity:critical` share the same label-write trust surface — any repo collaborator with Triage-role access or higher can apply any of these labels to an arbitrary open issue and trigger autonomous implementation without the `ready-for-implementer` gate. This is consciously accepted while the fleet has no human collaborators beyond the maintainer. **Precondition for granting triage+ access to any human collaborator:** ship sender-verification enforcement (a labels workflow that validates the integration sender) to close this surface before expanding access.
...
- **You realize mid-implementation that the change requires an ADR** (a **compositional self-change** — per ADR-0047, a change that *expands loop capability or weakens a control*: it touches the team's gates/dispatch/review machinery, the guard scripts, intake/deploy/branch protection, `CODEOWNERS`, a rail-enforcer agent, the standards or ADRs, adds a new external dependency, or runs a destructive migration): stop. Do **not** build it and do **not** draft the ADR. **Routine/mechanical platform fixes are NOT compositional** — hardening, SHA-pins, additive guards, additive columns, cosmetic edits: build and ship them like any project fix. Likewise apply `requires-adr:schema`/`requires-adr:api-contract` only for genuinely new architecture (a new data store/entity/relationship, a new or changed public API contract), never for an additive change that merely enforces an already-decided model. Route for human formulation using the path that matches your execution context:
  - **Platform repo** (`agentic-dev-environment`): file a platform-repo issue labelled `needs-formulation` + `requires-adr` + `compositional-self-change` describing the proposed change; it enters human formulation (ADR-0036/0038), where the architect drafts the ADR and the human ratifies by approving it. Post a comment on the originating issue explaining that a compositional self-change was detected, that the implementer has stopped without building, and that a platform-repo issue has been filed per ADR-0036/0038.
  - **Project repo**: do **not** attempt `gh issue create` against the platform repo — project-repo agents hold no write access there (ADR-0019 sub-decision 6). Instead, apply the `process-flaw` label to the current project issue and post a comment explaining the detection; triage-scan will pick up the `process-flaw` label and file the platform-repo issue via the one-directional pull mechanism (ADR-0019 sub-decision 2).
