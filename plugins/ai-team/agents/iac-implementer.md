...
- ❌ **Introducing deprecated fleet terms.** The canonical glossary at `docs/standards/00-terminology.md` lists fleet-specific terms (Fleet App slug, etc.). Consult it before naming Terraform variables, resource names, or comments with fleet-specific identifiers. Same discipline as the application implementer. (`terminology-check` CI enforcement is not yet active — sweeps are manual until it is.)
