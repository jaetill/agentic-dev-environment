...
2. **If you cannot disambiguate, close it and fix the source.**
   ...
   - That agent-quality issue is **exempt from the survived-one-cycle gate** — it is promoter-authored and well-specified by construction — so **promote and dispatch it in this same run**, counting it against `$FLEET_MAX_DISPATCH` like any other promotion. The implementer drafts the contract tightening as a human-reviewed PR. (If the cap is genuinely full this run, leave it promoted-but-undispatched as an ordinary ticket; it is already well-specified and dispatches next cycle — one per agent, deduped, so it cannot pile up.)
