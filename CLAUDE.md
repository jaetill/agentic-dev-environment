...
- **Use the canonical glossary at `docs/standards/00-terminology.md`** as the source of truth for fleet-specific terms (Fleet App slug, implementer PR author, deprecated terms). `terminology-check` (PR #364) blocks PRs that reintroduce retired terms (e.g. `app/claude`, `IMPLEMENTER_PAT` as the implementer's auth path) in operational files. When you retire a term, update the glossary first; the check then surfaces every downstream reference that needs updating in the same PR.
