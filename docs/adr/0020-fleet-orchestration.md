...
- **Sub-decision 2 → Dispatch-triggered.** The promoter applies `ready-for-implementer` as durable state and then explicitly dispatches the target repo's `claude-implementer.yml` via `workflow_dispatch`. The label records intent; the dispatch is the trigger.
...
- **Affected workflows:** `triage-scan.yml` (fleet scan + promote + dispatch + spare-capacity sweep); `ci-health.yml` (widen the watch from platform-repo-only to the whole fleet); each project's `claude-implementer.yml` caller stub (accept a dispatched `issue_number`; emit the two-PR sweep).
