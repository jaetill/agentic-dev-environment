...
> **Clarifications (2026-06-20), addressing #52 and #54:**
> - **Condition 1 — implementer identity (#54).** "Authored by the implementer agent" means a concrete GitHub actor, not a description: implementer fix PRs are authored by the fleet App bot **`jaetill-ai-triage-team[bot]`** (per [ADR-0041](0041-fleet-app-single-write-identity.md)). The deprecated `app/claude` author appears only on pre-migration PRs; identity checks must OR both across the migration seam and treat `app/claude` as deprecated (see `docs/standards/00-terminology.md`). The gate must match this exact identity so that an arbitrary bot cannot satisfy condition 1.
> - **Condition 3 — requires-adr blind-spot (#52).** Condition 3 routes the ADR-gated categories — notably **security-relevant** changes — to the human *via the `requires-adr:*` label*. This is a detection dependency: a security-relevant change whose label is never applied would satisfy condition 3 vacuously and auto-merge. That residual risk is now closed at merge time by [ADR-0047](0047-firewall-gates-on-capability-delta.md)'s capability-delta firewall, which holds any PR that adds capability or weakens a control **regardless of the `requires-adr` label** — so the label is a routing convenience, not the sole security boundary.
