...
- **Branch protection** — `required_conversation_resolution` OFF fleet-wide (it silently blocks autonomous merge). `enforce_admins` is **retained as an independent security control**: `enforce_admins: true` forces even admin tokens (including the fleet app token) through the PR + AI-review battery. `required_signatures` is **aspirational, NOT currently enforced** (corrected per #104): GitHub exposes it only via the separate `/branches/{branch}/protection/required_signatures` endpoint, which `configure-branch-protection.ps1` does not call — and turning it on would reject the loop's own unsigned `claude[bot]` git-push commits, breaking autonomous merge. The active direct-push controls are therefore `enforce_admins` + the required CI/AI-review check battery; commit-signature enforcement would first require the loop to produce signed commits (e.g. via the App's commit API, which GitHub signs). Only `required_conversation_resolution` (the autonomous-merge blocker) is turned off. Standard 01 §6 and `scripts/configure-branch-protection.ps1` updated; ADR-0002's "Strict" branch-protection sub-decision is amended to match.
