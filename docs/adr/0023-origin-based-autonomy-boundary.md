...
- **Branch protection** — `required_conversation_resolution` OFF fleet-wide (it silently blocks autonomous merge). `enforce_admins` and `required_signatures` are reclassified as plumbing, not human-control gates; the required status *checks* are unchanged. Standard 01 §6 and `scripts/configure-branch-protection.ps1` updated; ADR-0002's "Strict" branch-protection sub-decision is amended to match.
