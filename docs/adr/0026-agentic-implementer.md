...
- **Scope cap.** A PR may not exceed **50 lines of production code, 3 source files, or 1 component**. Over the cap, the implementer **decomposes the problem itself and ships the smallest coherent slice**, tracking the remainder — it does not refuse or escalate size to a human (amended 2026-06-01; see top). (Tests and docs don't count toward the LOC cap.)
