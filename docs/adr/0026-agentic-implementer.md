...
> **Extended by [ADR-0042](0042-autonomous-conflict-requeue.md):** the optimistic-concurrency model gains autonomous recovery — a pre-flight rebase conflict (or merge-time conflict) self-re-queues the issue (strip `ready-for-implementer` + `conflict-retry:<n>`, cap 7, then `human-todo` escalation) instead of commenting for a human re-poke. The no-LLM-conflict-resolution boundary is unchanged.
