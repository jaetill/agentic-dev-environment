...
- **Guard** — `scripts/additive-self-change-guard.sh` (generalized in place from the ADR-0032 lane; filename kept so the `triage-scan.yml` reference stays valid). Deterministic, no LLM: **scoped to the platform repo via `IS_PLATFORM`** (app-repo changes are never self-changes, ADR-0019); gate-machinery matched by sensitivity *pattern* not exact filename; guardrail-vocabulary denylist over `git diff` added+removed lines; destructive-migration detector (universal); pure-SHA-pin carve-out; non-pin-workflow hold; safety-pragma-removal hold; fail-closed on unavailable diff or a possibly-truncated (≥100) file list; a generous changed-line backstop cap. Verdict: `HOLD` (→ `hold:compositional`) or `IN_LANE`. Unit-tested in `scripts/test-additive-self-change-guard.sh` (29 cases: gate-machinery/hooks/skills held, implementer agent held, control-weakening held, additive column & operational hardening in-lane, SHA-pin in-lane, destructive migration held, app-repo firewall-skip, H1/H3 fail-closed).
...
- **Standards** — `docs/standards/10-ai-workflows.md` (auto-merge condition 4) and `docs/standards/12-self-modification.md`: define "compositional" as the capability delta. (A canonical glossary entry in `docs/standards/00-terminology.md` is deferred to the in-flight terminology ADR that introduces that file; this ADR does not create it.)
