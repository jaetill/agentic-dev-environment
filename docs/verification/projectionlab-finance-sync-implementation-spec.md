...
- Your `plan.json` (generated by Claude from your `jason_finance.md` memo;
  see `plan-regeneration.md` in the target repo)
...
If using the autonomous-team model (ADR-0026 per game-night-pwa), break this spec into implementer-sized tickets (400 LOC / 8 files / 3 components cap each — per ADR-0026 current). Suggested decomposition:
...
- Read this spec.
- Open ticket #1, implement it within the scope cap (400 LOC / 8 files / 3 components — per ADR-0026 current), open a PR, let code-reviewer + security-reviewer run, address blocking findings, get merged.
