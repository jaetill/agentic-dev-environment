...
> **Do _not_ require conversation resolution before merging** — `required_conversation_resolution` stays **off**. Per the autonomous merge policy ([ADR-0021](../adr/0021-autonomous-merge.md) as amended by ADR-0039, pending — PR #180), the autonomous auto-merge (`gh pr merge --squash`) doesn't resolve PR review threads; if AI reviewers post any comment thread, an enabled repo blocks the loop's merge silently. Off everywhere fleet-wide.
