...
Low and nit findings get filed as GitHub issues at `severity:low`/`severity:nit`; the nit tier is the deferred tier, no separate label (ADR-0037). The deferred set is `label:severity:low,severity:nit -label:ready-for-implementer`. The implementer does NOT pick them up in isolation. Two distinct mechanisms drain them (per **[ADR-0020](../adr/0020-fleet-orchestration.md)**/[ADR-0029](../adr/0029-promoter-owns-nit-adjacency.md), amending ADR-0016's flat cap of 2): (1) a dispatched **fix PR** rides along directory-adjacent deferred nits the promoter pre-selected — the **adjacency bundle**, cap `min(floor(total / 2), 4)`, limited to nits in a file the fix actually touched; and (2) the promoter's spare-capacity **`mode=cleanup-sweep`** dispatch opens a *separate* cleanup PR draining a bounded batch of the remaining low/nit issues, cap `max(floor(total / 2), 8)`, chunked at 12 per PR, on repos that get no qualifying feature work. `total` is the repo's open low/nit count. (The Mode-A per-PR "sidecar" was retired by #119 — adjacency-bundle ride-along plus the separate cleanup-sweep replace it — so cold-code nits no longer wait on coincidental adjacency.)
...
1. **Implementer-authored and linked issue present.** The PR's author is the fleet App (`gh pr list --json author` returns `author.is_bot == true` with `author.login` matching `jaetill-ai-triage-team` — either `app/jaetill-ai-triage-team` or `jaetill-ai-triage-team[bot]` depending on the gh CLI version, per ADR-0041) and the PR body links an issue (`Closes/Fixes #n`) for traceability. Origin no longer gates the merge (ADR-0039) — a bug the maintainer filed and a Sentry-detected error merge the same way, including `approved` features (ADR-0036), provided every other condition holds.
...
3. **No `requires-adr:*` label.** The five ADR-gated categories (ADR-0003) route to the human. Per ADR-0047 the categories apply to *genuine architectural decisions* — a new data store/entity/relationship, a new or changed public API contract, a new external dependency, a destructive migration — never to an additive, restrictive change that merely *enforces an already-decided model* (adding `owner_email` to scope a query; adding an auth guard to an existing route). Enforcing an existing model is implementation, not architecture.
4. **The capability-delta firewall holds (ADR-0047).** Routine/mechanical fixes — additive guards, additive columns, SHA-pins, cosmetic/doc/agent-logic changes — are eligible like any project fix. A green PR holds for human ratification (`hold:compositional`) only when it *expands loop capability or weakens a control*: a changed gate/governance file (the auto-merger, dispatch, review gates, the guards, intake admission, deploy/branch protection, `CODEOWNERS`, the rail-enforcer agents, or `docs/adr`+`docs/standards`), guardrail vocabulary on an added/removed line, a net-new external action, or a destructive migration. This is decided by the **deterministic `additive-self-change-guard.sh`, authoritative over the advisory `compositional-self-change` label** (ADR-0047 generalizes the ADR-0032 lane and supersedes the ADR-0023 label-based hold).
