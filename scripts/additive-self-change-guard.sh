...
GATE_MACHINERY='^\.github/workflows/(triage-scan|claude-implementer.*|urgent-poll|pr-review|claude-pr-review|iac-guard|intake-steward|deploy-prod|emergency-deploy|validate-platform|security-scan)\.yml$|^scripts/.*(guard|protection|review-gate|validate-platform|intake-steward|fleet-inflight|configure).*|^(\.github/)?CODEOWNERS$|^docs/adr/|^docs/standards/|^plugins/ai-team/(hooks|skills|commands)/|^plugins/ai-team/agents/(triage-bot|architect|security-reviewer|code-reviewer)\.md$'
