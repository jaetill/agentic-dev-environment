...
# (.github/workflows/), standards (docs/standards/), ADRs (docs/adr/), the
# agent-team hierarchy (plugins/ai-team/), or the guard scripts themselves
# (scripts/) is a compositional self-change requiring human review. Agent files
# (plugins/ai-team/agents/*.md) are handled below by the ADR-0032 additive
# self-change lane and are skipped here.
while IFS= read -r f; do
  [[ -z "$f" ]] && continue
  [[ "$f" =~ ^plugins/ai-team/agents/[^/]+\.md$ ]] && continue  # ADR-0032 lane
  if [[ "$f" =~ ^\.github/workflows/ || "$f" =~ ^docs/(standards|adr)/ \
     || "$f" =~ ^plugins/ai-team/ || "$f" =~ ^scripts/ ]]; then
